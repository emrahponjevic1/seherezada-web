...
import { IkonaKlik, IkonaSken, IkonaVanjski } from "@/app/(statistika)/_qr/IkonePanela";
...
// STRAN LINKTREEJA
//
// Dva zavihka: "Uredi" (privzeto — kot pri Linktree adminu) in "Statistika".
//
...
export default async function LinktreePage({
  ...
  const prikaz = trazeniPrikaz === "statistika" ? "statistika" : "uredi";
  const javniNaslov = `${bazniUrl()}/links/${stranica.slug}`;
  const urlUredjivanja = `/statistika/linkovi/${stranica.id}`;
  const urlStatistike = `/statistika/linkovi/${stranica.id}?prikaz=statistika${r ? `&raspon=${raspon.kljuc}` : ""}`;
      ...
      <Zaglavlje
        ...
        podnaslov={
          <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
            <a href={javniNaslov} target="_blank" rel="noreferrer" className={s.kodLinkVeliki}>
        ...
        meta={
          <>
            ...
            {stranica.glavna && <span className={`${s.cip} ${s.cipNaglasak}`}>Glavna</span>}
          </>
        }
      >
        <a href={javniNaslov} target="_blank" rel="noreferrer" className={s.dugmeSporedno}>
          <IkonaVanjski velicina={17} />
          Otvori stranicu
        </a>
      </Zaglavlje>
  ...
  return (
    <main className={s.sekcija}>
      <div className={s.kontejner}>
        ...
        <FilterRaspona putanja={`/statistika/linkovi/${stranica.id}?prikaz=statistika`} aktivan={raspon} />
        ...
        <section className={s.kartica}>
          <div className={s.karticaGlava}>
            <h2 className={s.karticaNaslov}>Klikovi {dani.jedinica === "week" ? "po sedmicama" : "po danima"}</h2>
            {klikovi.boti > 0 && <span className={s.pomoc}>Isključeno {broj(klikovi.boti)} otvaranja od botova.</span>}
          </div>
          <SlojeviGrafikon
        ...
        <section className={s.kartica}>
          ...
          {dugmad.length === 0 ? (
            ...
            <ul className={l.tabelaDugmadi}>
              {dugmad.map((d) => {
                const udio = otvaranja.otvaranja > 0 ? Math.round((d.u_rasponu / otvaranja.otvaranja) * 100) : 0;
                return (
                  <li key={d.id}>
                    <Link href={`/statistika/${d.id}`} className={`${l.dugmeRed} ${d.aktivan ? "" : l.dugmeUgasen}`}>
        ...
        <section className={s.kartica}>
          <div className={s.karticaGlava}>
            <h2 className={s.karticaNaslov}>QR kodovi koji vode ovamo</h2>
          </div>
          {kodovi.length === 0 ? (
            <div className={s.prazno}>
              <p>Nijedan još. Napravi QR kod i za odredište izaberi ovu stranicu.</p>
            ...
            <ul className={l.tabelaDugmadi}>
              {kodovi.map((k) => (
                <li key={k.id}>
                  <Link href={`/statistika/${k.id}`} className={l.dugmeRed}>
                    <span />
                    <span className={l.dugmeInfo}>
                      <span className={l.dugmeNatpis}>{k.naziv}</span>
                      <span className={l.dugmeCilj}>/q/{k.slug}</span>
                    </span>
                    <span className={l.dugmeBroj}>{broj(k.u_rasponu)}</span>
                    <span className={l.dugmeUdio}>ukupno {broj(k.ukupno)}</span>
                    <span />
                    <span className={p.kodStrelica} aria-hidden="true">
                      →
                    </span>
